                    ...
                    if (preg_match('/^pick-and-drop\/(\d+)$/', $path, $matches)) {
                        ...
                        if ($service) {
                            $title = sprintf(
                                ...
                                htmlspecialchars($service->start_location ?? '', ENT_QUOTES, 'UTF-8'),
                                htmlspecialchars($service->end_location ?? '', ENT_QUOTES, 'UTF-8')
                            );
                            
                            $descriptionParts = [];
                            $descriptionParts[] = sprintf('🚗 %s Driver Available', ucfirst($service->driver_gender ?? ''));
                            $descriptionParts[] = sprintf('📍 Route: %s → %s', htmlspecialchars($service->start_location ?? '', ENT_QUOTES, 'UTF-8'), htmlspecialchars($service->end_location ?? '', ENT_QUOTES, 'UTF-8'));
                            $descriptionParts[] = sprintf('⏰ %s at %s', ($service->is_everyday ?? false) ? 'Everyday' : 'Scheduled', htmlspecialchars($service->departure_time ?? '', ENT_QUOTES, 'UTF-8'));
                            $descriptionParts[] = sprintf('👥 %d Space%s Available', $service->available_spaces ?? 0, ($service->available_spaces ?? 0) !== 1 ? 's' : '');
                            
                            if (!empty($service->price_per_person)) {
                                $descriptionParts[] = sprintf('💰 %s %s per person', htmlspecialchars($service->currency ?? 'PKR', ENT_QUOTES, 'UTF-8'), number_format($service->price_per_person, 0));
                            }
